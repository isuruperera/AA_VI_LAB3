...
#include <stdlib.h>
#include <time.h>
void matmul_simple_av(int n, float *mat_c[n],
...
void test_mat_mul_simple_av(int n, float *mat_c[n],
                            const float *mat_a[n], const float *mat_b[n], int iterations){
    double totalTime = 0;
    for(int i=0; i<iterations ;i++)
    {
        clock_t start = clock();
        matmul_simple_av(n,mat_c,mat_a,mat_b);
        clock_t end = clock();
...
void test_all_mat_mul_simple_av(){
    for(int n=100; n<=1600 ;n*=2)
    {
        ...
        test_mat_mul_simple_av(n, mat_c, (const float **) mat_a, (const float **) mat_b, 10);
        for(int j=0;j<n;j++){
            free(mat_a[j]);
            free(mat_b[j]);
            free(mat_c[j]);
